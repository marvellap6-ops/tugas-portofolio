...
def create_app():
    ...
    @app.route('/admin')
    def admin_index():
        # Redirect (bukan render langsung) agar URL punya nama file yang jelas,
        # supaya path relatif seperti "css/main.css" / "js/main.js" resolve dengan benar.
...
# Objek `app` di level modul -- ini yang dipanggil Gunicorn saat production
# lewat `gunicorn app:app` (bukan cuma lewat `python app.py`).
app = create_app()
